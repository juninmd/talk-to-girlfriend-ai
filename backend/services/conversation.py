...
from backend.config import (
    CONVERSATION_MIN_DELAY,
    CONVERSATION_MAX_DELAY,
    CONVERSATION_TYPING_SPEED,
)
...
class ConversationService:
    ...
    async def handle_incoming_message(self, event):
        ...
        try:
            ...
            if (
                event.sender
                and isinstance(event.sender, User)
            ...
            # Trigger reply
            asyncio.create_task(
                self._generate_and_send_reply(chat_id, text, sender_name)
            )
    ...
    async def _generate_and_send_reply(
        self, chat_id: int, user_message: str, sender_name: str
    ):
        ...
        try:
            ...
            delay = min(
                ...
                max(
                    CONVERSATION_MIN_DELAY,
                    len(user_message) * CONVERSATION_TYPING_SPEED,
                ),
            )
            ...
            async with self.client.action(chat_id, "typing"):
                ...
                typing_delay = min(
                    CONVERSATION_MAX_DELAY * 1.5,
                    len(response_text) * CONVERSATION_TYPING_SPEED,
                )
