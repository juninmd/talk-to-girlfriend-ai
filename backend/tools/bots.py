...
async def get_bot_info(bot_username: str) -> str:
    try:
        ...
        if not entity: return f"Bot {bot_username} not found."
...
async def set_bot_commands(bot_username: str, commands: list) -> str:
    try:
        ...
        if not getattr(me, "bot", False): return "Error: This function is for bot accounts only."
        bot_commands = [BotCommand(command=c["command"], description=c["description"]) for c in commands]
        await client(SetBotCommandsRequest(scope=BotCommandScopeDefault(), lang_code="en", commands=bot_commands))
