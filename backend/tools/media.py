...
async def send_file(chat_id: Union[int, str], file_path: str, caption: str = None) -> str:
    try:
        ...
        if not os.access(file_path, os.R_OK): return f"File is not readable: {file_path}"
...
async def download_media(chat_id: Union[int, str], message_id: int, file_path: str) -> str:
    try:
        ...
        if not msg or not msg.media: return "No media found in the specified message."
        dir_path = os.path.dirname(file_path) or "."
        if not os.access(dir_path, os.W_OK): return f"Directory not writable: {dir_path}"
        await client.download_media(msg, file=file_path)
        if not os.path.isfile(file_path): return f"Download failed."
...
async def send_voice(chat_id: Union[int, str], file_path: str) -> str:
    try:
        if not os.path.isfile(file_path): return f"File not found."
