...
async def get_messages(chat_id: Union[int, str], page: int = 1, page_size: int = 20) -> str:
    try:
        ...
        if not messages: return "No messages found for this page."
        lines = []
        for msg in messages:
...
@validate_id("chat_id")
async def list_messages(chat_id: Union[int, str], limit: int = 20, search_query: str = None, from_date: str = None, to_date: str = None) -> str:
    try:
        entity = await client.get_entity(chat_id)
        ...
        for msg in messages:
            ...
            lines.append(f"ID: {msg.id} | {sender_name} | Date: {msg.date} | Message: {message_text}")
        return "\n".join(lines)
    except Exception as e:
