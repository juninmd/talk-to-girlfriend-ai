...
from telethon.tl.types import ChatAdminRights, ChatBannedRights, ChannelParticipantsAdmins, ChannelParticipantsKicked
...
async def promote_admin(group_id: Union[int, str], user_id: Union[int, str], rights: dict = None) -> str:
    try:
        ...
        admin_rights = ChatAdminRights(**{k: v for k, v in rights.items() if hasattr(ChatAdminRights, k)})

        try:
            await client(functions.channels.EditAdminRequest(channel=chat, user_id=user, admin_rights=admin_rights, rank="Admin"))
...
async def demote_admin(group_id: Union[int, str], user_id: Union[int, str]) -> str:
    try:
        chat = await client.get_entity(group_id)
        user = await client.get_entity(user_id)
        admin_rights = ChatAdminRights(change_info=False, post_messages=False, edit_messages=False, delete_messages=False, ban_users=False, invite_users=False, pin_messages=False, add_admins=False, anonymous=False, manage_call=False, other=False)
        try:
            await client(functions.channels.EditAdminRequest(channel=chat, user_id=user, admin_rights=admin_rights, rank=""))
...
async def ban_user(chat_id: Union[int, str], user_id: Union[int, str]) -> str:
    try:
        ...
        user = await client.get_entity(user_id)
        banned_rights = ChatBannedRights(until_date=None, view_messages=True, send_messages=True, send_media=True, send_stickers=True, send_gifs=True, send_games=True, send_inline=True, embed_links=True, send_polls=True, change_info=True, invite_users=True, pin_messages=True)
        try:
            await client(functions.channels.EditBannedRequest(channel=chat, participant=user, banned_rights=banned_rights))
...
async def unban_user(chat_id: Union[int, str], user_id: Union[int, str]) -> str:
    try:
        chat = await client.get_entity(chat_id)
        user = await client.get_entity(user_id)
        unbanned_rights = ChatBannedRights(until_date=None, view_messages=False, send_messages=False, send_media=False, send_stickers=False, send_gifs=False, send_games=False, send_inline=False, embed_links=False, send_polls=False, change_info=False, invite_users=False, pin_messages=False)
        try:
...
async def get_recent_actions(chat_id: Union[int, str]) -> str:
    try:
        ...
        if not result or not result.events: return "No recent admin actions found."
