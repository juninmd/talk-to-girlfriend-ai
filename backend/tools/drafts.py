...
@validate_id("chat_id")
async def save_draft(chat_id: Union[int, str], message: str, reply_to_msg_id: Optional[int] = None, no_webpage: bool = False) -> str:
    try:
        ...
        await client(functions.messages.SaveDraftRequest(peer=peer, message=message, no_webpage=no_webpage, reply_to=reply_to))
...
async def get_drafts() -> str:
    try:
            ...
            for update in result.updates:
                if hasattr(update, "draft") and update.draft:
                    ...
                    if hasattr(update, "peer"):
                        peer = update.peer
                        if hasattr(peer, "user_id"): peer_id = peer.user_id
                        elif hasattr(peer, "chat_id"): peer_id = -peer.chat_id
                        elif hasattr(peer, "channel_id"): peer_id = -1000000000000 - peer.channel_id
        ...
        if not drafts_info: return "No drafts found."
