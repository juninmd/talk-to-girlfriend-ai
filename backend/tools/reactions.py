...
@validate_id("chat_id")
async def send_reaction(chat_id: Union[int, str], message_id: int, emoji: str, big: bool = False) -> str:
    try:
        peer = await client.get_input_entity(chat_id)
        await client(functions.messages.SendReactionRequest(peer=peer, msg_id=message_id, big=big, reaction=[ReactionEmoji(emoticon=emoji)]))
...
async def remove_reaction(chat_id: Union[int, str], message_id: int) -> str:
    try:
        peer = await client.get_input_entity(chat_id)
        await client(functions.messages.SendReactionRequest(peer=peer, msg_id=message_id, reaction=[]))
...
async def get_message_reactions(chat_id: Union[int, str], message_id: int, limit: int = 50) -> str:
    try:
        peer = await client.get_input_entity(chat_id)
        result = await client(functions.messages.GetMessageReactionsListRequest(peer=peer, id=message_id, limit=limit))
        if not result.reactions: return f"No reactions on message {message_id}."

        data = []
        for r in result.reactions:
            user_id = r.peer_id.user_id if hasattr(r.peer_id, "user_id") else None
            emoji = r.reaction.emoticon if isinstance(r.reaction, ReactionEmoji) else f"custom:{r.reaction.document_id}"
            data.append({"user_id": user_id, "emoji": emoji, "date": r.date.isoformat() if r.date else None})
