...
async def send_sticker(chat_id: Union[int, str], file_path: str) -> str:
    try:
        if not os.path.isfile(file_path): return f"Sticker file not found: {file_path}"
        if not file_path.lower().endswith(".webp"): return "Sticker file must be .webp file."
    ...
    try:
        try:
            ...
            result = await client(functions.messages.SearchRequest(
                peer="gif", q=query, filter=InputMessagesFilterGif(), min_date=None, max_date=None,
                offset_id=0, add_offset=0, limit=limit, max_id=0, min_id=0, hash=0
            ))
            if not result or not hasattr(result, "messages") or not result.messages: return "[]"
            gif_ids = []
...
async def send_gif(chat_id: Union[int, str], gif_id: int) -> str:
    try:
        if not isinstance(gif_id, int): return "gif_id must be an integer (document ID)."
        entity = await client.get_entity(chat_id)
