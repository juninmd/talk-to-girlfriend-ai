...
def _format_dialog_info(dialog):
    """Helper to format dialog info for list_chats."""
    entity = dialog.entity
    current_type = None
    ...
    return entity, current_type


def _build_chat_info_string(entity, current_type, dialog):
    """Helper to build chat info string."""
...
async def list_chats(chat_type: str = None, limit: int = 20) -> str:
    ...
    try:
        ...
        for dialog in dialogs:
            entity, current_type = _format_dialog_info(dialog)

            if chat_type and current_type != chat_type.lower():
                continue

            results.append(
                _build_chat_info_string(entity, current_type, dialog)
            )
...
async def _format_entity_details(entity):
    """Helper to format entity details for get_chat."""
    result = []
...
async def leave_chat(chat_id: Union[int, str]) -> str:
    ...
    try:
        ...
        if isinstance(entity, Channel):
            ...
            return await _leave_chat_basic(entity, chat_id)
        else:
            return log_and_format_error(
                "leave_chat",
                Exception(
