...
2.  **Frontend/Agente (TypeScript)**: Uma aplicação de linha de comando (`agent/`) que interage com o usuário, processa comandos em linguagem natural usando IA (Claude) e executa ações no Telegram através da API REST do backend.
...
*   **MCP Server**: Permite que ferramentas de IA como Claude Desktop ou Cursor se conectem diretamente ao seu Telegram.
...
*   **Vercel AI SDK**: Usa `streamText` da biblioteca `ai` para se comunicar com o modelo de linguagem (Claude).
...
*   **Fluxo**:
    ...
    2.  Envia histórico para o Claude.
    3.  Claude decide se precisa chamar uma ferramenta (ex: ler mensagens, buscar cantada).
    4.  Se chamar ferramenta, o código executa a função correspondente (que chama a API do Python) e devolve o resultado para o Claude.
    5.  Claude gera a resposta final para o usuário.
...
3.  **Claude** (em `agent.ts`) analisa o texto e decide chamar a ferramenta `getChats` ou `getMessages`.
...
6.  **Claude** recebe o JSON, interpreta as mensagens e gera uma resposta em texto: "A última mensagem da Maria foi 'Oi, tudo bem?'".
