...
async def test_reporting_grouping_logic():
    ...
    msg1 = Message(
        telegram_message_id=1, chat_id=100, sender_id=1, sender_name="Alice",
        text="Hello", date=datetime.now(timezone.utc), is_outgoing=False
    )
    msg2 = Message(
        telegram_message_id=2, chat_id=100, sender_id=2, sender_name="Bob",
        text="Hi Alice", date=datetime.now(timezone.utc), is_outgoing=False
    )
    msg3 = Message(
        telegram_message_id=3, chat_id=200, sender_id=3, sender_name="Charlie",
        text="Work update", date=datetime.now(timezone.utc), is_outgoing=False
    )

    # Mock dependencies
    with patch.object(reporting_service, '_fetch_messages_for_report', return_value=[msg1, msg2, msg3]):
        with patch('backend.services.reporting.ai_service.summarize_conversations', new_callable=AsyncMock) as mock_summarize:
            with patch('backend.services.reporting.client') as mock_client:
                ...
                async def get_entity_side_effect(chat_id):
                    if chat_id == 100: return mock_entity_100
                    if chat_id == 200: return mock_entity_200
                    if chat_id == 999: return MagicMock() # REPORT_CHANNEL_ID
