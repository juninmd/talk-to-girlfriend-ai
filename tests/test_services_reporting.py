...
@pytest.mark.asyncio
async def test_generate_daily_report_with_messages(
    mock_session, mock_client, mock_ai_service
):
    # Mock messages
    mock_msg = MagicMock()
    ...
    with patch("asyncio.to_thread", new_callable=AsyncMock) as mock_thread:
        ...
        mock_ai_service.summarize_conversations = AsyncMock(
...
@pytest.mark.asyncio
async def test_generate_daily_report_no_channel_id(
    mock_session, mock_client, mock_ai_service
):
    mock_msg = MagicMock()
    with patch("asyncio.to_thread", new_callable=AsyncMock) as mock_thread:
