...
def test_api_exceptions(mock_telegram_service):
    ...
    methods = [
        "get_me", "get_chats", "get_chat", "get_messages", "send_message",
        "schedule_message", "send_file", "get_contacts", "search_contacts",
        "send_reaction", "reply_to_message", "edit_message", "delete_message",
        "forward_message", "mark_as_read", "pin_message", "search_messages",
        "get_user_status", "get_user_photos", "search_gifs"
    ]
    ...
    assert client.post("/chats/1/schedule", json={"message": "hi", "minutes_from_now": 1}).status_code == 500
