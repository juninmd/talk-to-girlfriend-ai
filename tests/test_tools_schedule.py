...
async def test_schedule_message(mock_client):
    ...
    result = await schedule.schedule_message(chat_id=123, message="Test", minutes_from_now=10)
    assert "Message scheduled" in result


@pytest.mark.asyncio
async def test_schedule_message_validation(mock_client):
    result = await schedule.schedule_message(chat_id=123, message="Test", minutes_from_now=0)
    assert "must be at least 1" in result

    result = await schedule.schedule_message(chat_id=123, message="Test", minutes_from_now=1000000)
